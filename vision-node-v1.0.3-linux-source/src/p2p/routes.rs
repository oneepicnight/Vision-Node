                    ...
                    match crate::chain::accept::apply_block(&mut g, &block, peer_str) {
                        Ok(()) => {
                            ...
                            tracing::info!(
                                ...
                                chain_id = %chain_id,
                                pow_fp = %pow_fp,
                                "[ACCEPT] Compact block accepted - miner rewarded"
                            );

                            eprintln!(
                                "✅ Block #{} accepted from miner '{}' via compact block (peer: {:?})",
                                new_height,
                                block.header.miner,
                                peer_address
                            );
                        ...
                        Err(e) => {
                            ...
                            tracing::error!(
                                ...
                                chain_id = %chain_id,
                                pow_fp = %pow_fp,
                                "[REJECT] Compact block rejected from peer"
                            );
                            
                            // STRIKE: Invalid compact block (bad POW or invalid structure)
                            if let Some(ref peer) = peer_address {
                                let is_pow_error = e.contains("pow_hash mismatch") || e.contains("digest") || e.contains("bad_pow");
                                let peer_addr = peer.clone();
                                let strike_reason = if is_pow_error { "bad_pow" } else { "invalid_block" };
                                tokio::spawn(async move {
                                    crate::PEER_MANAGER.add_peer_strike(&peer_addr, strike_reason).await;
                                });
                            }
...
pub async fn announce_compact_block_to_peers(block: &crate::Block) {
    ...
    // HTTP fallback disabled - compact blocks only via P2P TCP connections
    tracing::debug!(
        target: "p2p::compact",
        hash = %block.header.pow_hash,
        height = block.header.number,
        "HTTP fallback disabled - relying on P2P TCP only"
    );
}
