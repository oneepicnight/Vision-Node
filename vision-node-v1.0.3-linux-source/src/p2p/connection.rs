...
/// ⭐ Node Version for MAINNET v1.0.3 (103 = v1.0.3)
pub const VISION_NODE_VERSION: u32 = 103;

/// ⭐ Node Build Tag - Must match for P2P compatibility
pub const NODE_BUILD_TAG: &str = crate::vision_constants::VISION_VERSION;

/// ⭐ Compute POW params consensus fingerprint (Fix A)
/// Returns hex hash of VisionX parameters to prevent algorithm drift between peers
pub fn compute_pow_params_hash() -> String {
    use sha2::{Sha256, Digest};
    
    let params = &crate::consensus_pow::params::VISIONX_CONSENSUS_PARAMS;
    let visionx = crate::consensus_pow::params::consensus_params_to_visionx(params);
    
    // Stable fingerprint format
    let fingerprint = format!(
        "v1:dataset_mb={}:scratch_mb={}:mix_iters={}:reads_per_iter={}:write_every={}:epoch_blocks={}",
        visionx.dataset_mb,
        visionx.scratch_mb,
        visionx.mix_iters,
        visionx.reads_per_iter,
        visionx.write_every,
        visionx.epoch_blocks
    );
...
/// ⭐ POW message version (block encoding format)
/// This must match across network or blocks won't decode correctly
pub const POW_MSG_VERSION: u32 = 150; // Current encoding version
...
pub struct HandshakeMessage {
    ...
    /// PoW params consensus fingerprint (dataset_mb, scratch_mb, mix_iters, reads_per_iter, write_every, epoch_blocks)
    /// This MUST match to prevent accepting blocks from different PoW algorithms
    #[serde(default)]
    pub pow_params_hash: String, // Hex string - hash of VisionXParams
    
    /// PoW message version (e.g., 121 vs 150 bytes) for block encoding compatibility
    #[serde(default)]
    pub pow_msg_version: u32, // Must match or blocks won't decode correctly
}
...
impl HandshakeWireV3 {
    fn from_internal(h: &HandshakeMessage) -> Self {
        HandshakeWireV3 {
            ...
            pow_params_hash: if h.pow_params_hash.is_empty() { None } else { Some(h.pow_params_hash.clone()) },
            pow_msg_version: if h.pow_msg_version == 0 { None } else { Some(h.pow_msg_version) },
        }
    }
    ...
    fn try_into_internal(self) -> Result<HandshakeMessage, String> {
        ...
        Ok(HandshakeMessage {
            ...
            pow_params_hash: self.pow_params_hash.clone().unwrap_or_default(), // PoW params consensus
            pow_msg_version: self.pow_msg_version.unwrap_or(0), // PoW message version
        })
    }
}
...
impl HandshakeMessage {
    ...
    fn new() -> Result<Self, String> {
        ...
        Ok(HandshakeMessage {
            ...
            // Fix A: POW consensus fingerprints - CRITICAL for algorithm consensus
            pow_params_hash: compute_pow_params_hash(),
            pow_msg_version: POW_MSG_VERSION,
        })
    }
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if peer_genesis != expected_genesis {
            // **DIAGNOSTIC: Enhanced genesis mismatch logging**
            let local_compiled = expected_genesis_hex;
            let local_active = hex::encode(local_genesis.unwrap_or(expected_genesis));
            let remote = hex::encode(peer_genesis);
            
            error!(
                "[HANDSHAKE] REJECT: Genesis mismatch - local_compiled={} local_active={} remote={}",
                local_compiled, local_active, remote
            );
            
            return Err(format!(
                "❌ HANDSHAKE REJECT: GENESIS_HASH mismatch. local_compiled={} local_active={} remote={} \n\n\
                 This means the peer is on a different network or fork. \n\
                 Check: \n\
                 1. Both nodes running same binary/version? \n\
                 2. Port forwarding pointing to correct machine? \n\
                 3. Old node still running somewhere? \n",
                local_compiled,
                local_active,
                remote
            ));
...
mod tests {
    ...
    async fn handshake_rejects_different_genesis() {
        ...
        let h = HandshakeMessage {
            ...
            pow_params_hash: String::new(), // Empty for test
            pow_msg_version: 0, // Zero for test
        };
        let res = h.validate();
        assert!(res.is_err());
    }
...
impl P2PConnectionManager {
    ...
    async fn handle_inbound_connection(
        ...
        // SECURITY: Check if peer is quarantined (strike system)
        {
            let now = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
            ...
            let all_peers = crate::PEER_MANAGER.get_all_peers().await;
            if let Some(peer_record) = all_peers.iter().find(|p| format!("{}:{}", p.ip, p.port) == normalized_addr) {
                if peer_record.is_quarantined(now) {
                    tracing::warn!(
                        "[INBOUND-REJECT] ⛔ Quarantined peer attempted inbound connection peer={} strikes={}",
                        normalized_addr,
                        peer_record.strikes
                    );
                    return Ok(()); // Silently reject quarantined peers
                }
            }
        }
        ...
        */
        ...
        crate::PEER_MANAGER
            .update_peer_chain_identity(
                ...
                handshake.protocol_version,
                node_version_str,
                handshake.pow_params_hash.clone(),
                handshake.pow_msg_version,
                handshake.chain_height,
            )
            .await;
        ...
        */
        ...
        crate::PEER_MANAGER
            .update_peer_chain_identity(
                ...
                node_version_str,
                peer_handshake.pow_params_hash.clone(),
                peer_handshake.pow_msg_version,
                peer_handshake.chain_height,
            )
            .await;
        ...
        // FIX: Only send discovery messages to peers with protocol v2+ to prevent
        // crashing older peers with unknown message types (causes "early eof" disconnects)
        if peer_handshake.protocol_version >= 2 {
            // Request peer list for discovery
            if let Err(e) = self
                .send_to_peer(&normalized_addr, P2PMessage::GetPeers)
                .await
            {
                debug!(peer = %normalized_addr, protocol = peer_handshake.protocol_version, error = %e, 
                    "Failed to request peer list");
            }
            ...
            {
                debug!(peer = %normalized_addr, protocol = peer_handshake.protocol_version, error = %e, 
                    "Failed to send PEX request");
            }
        } else {
            debug!(
                peer = %normalized_addr, 
                protocol = peer_handshake.protocol_version,
                "Skipping GetPeers/PEX for legacy peer (protocol < 2)"
            );
        }
    ...
    async fn handle_peer_message(&self, address: &str, msg: P2PMessage) -> Result<(), String> {
        ...
        let (validated_chain, peer_exists) = {
            let peers = self.peers.lock().await;
            if let Some(peer_arc) = peers.get(address) {
                (peer_arc.lock().await.validated_chain, true)
            } else {
        ...
        if !validated_chain {
            match msg {
                ...
                _ => {
                    if peer_exists {
                        warn!(peer = %address, "⚠️ Dropping message from peer still validating handshake");
                    } else {
                        warn!(peer = %address, "🚫 Dropping stale message from disconnected peer");
                    }
                }
            }
            return Ok(());
        ...
        match msg {
            ...
            P2PMessage::GetBlocksByRange { start_height, max } => {
                ...
                // Record block request for sync state tracking
                crate::PEER_MANAGER.record_peer_block_request(address).await;
