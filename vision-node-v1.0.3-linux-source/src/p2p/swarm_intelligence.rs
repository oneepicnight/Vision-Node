...
impl PeerReputation {
    ...
    pub fn record_handshake_success(&mut self, latency_ms: u32) {
        ...
        // Log tier changes prominently
        if old_tier != self.tier {
            info!(
                "[SWARM] 🏆 Peer promoted: {} → {} (score={}, latency={}ms)",
                old_tier.label().to_uppercase(),
            ...
            debug!(
                "[SWARM] Peer reputation updated: tier={}, score={}, latency={}ms",
                self.tier.label(),
                self.score,
                latency_ms
            );
    ...
    pub fn record_synced_height(&mut self, height: u64, our_height: u64) {
        ...
        // Log demotion prominently
        if old_tier != self.tier && matches!(self.tier, ReputationTier::Poor | ReputationTier::Fair) {
            warn!(
                "[SWARM] ⚠️ Peer demoted: {} → {} (score={}, stale by {} blocks)",
                old_tier.label().to_uppercase(),
                self.tier.label().to_uppercase(),
                self.score,
                height_diff
