...
/// Peer sync state for directional connection management
/// Controls whether a peer counts toward mining gate and pruning policy
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum PeerSyncState {
    /// Peer is ahead of us - we sync from them (always keep, counts toward mining)
    Ahead,
    /// Peer is within ±10 blocks - mining consensus peer (always keep, counts toward mining)
    Consensus,
    /// Peer is behind us but actively requesting blocks (max 3, doesn't count toward mining)
    Syncing,
    /// Peer is behind us and not requesting blocks (prune if >100 blocks behind)
    Stale,
}
...
pub struct Peer {
    ...
    // Consensus validation fields (Fix A: Consensus fingerprints)
    pub pow_params_hash: Option<String>,
    pub pow_msg_version: Option<u32>,
    
    // Security and trust tracking (Fix B: Strike system)
    #[serde(default)]
    pub strikes: u32, // Bad PoW, invalid blocks, fork inconsistencies
    #[serde(default)]
    pub last_strike_at: Option<u64>, // Unix timestamp of last strike
    #[serde(default)]
    pub quarantined_until: Option<u64>, // Unix timestamp when quarantine expires
    
    // Validated height tracking (Fix C: Trusted heights)
    #[serde(default)]
    pub last_validated_height: u64, // Only increases when we validate blocks from this peer
    #[serde(default)]
    pub fork_consistent: bool, // Has peer shown consistency with our fork?
    
    // Directional sync state tracking
    #[serde(default)]
    pub sync_state: PeerSyncState, // Current sync state classification
    #[serde(default)]
    pub last_block_request_at: Option<u64>, // Unix timestamp of last GetBlocksByRange request
    #[serde(default)]
    pub connection_started_at: Option<u64>, // Unix timestamp when connection was established
}
...
impl Peer {
    ...
    /// Fix B: Add a strike to this peer for bad behavior (bad_pow, invalid_block, fork_inconsistency)
    pub fn add_strike(&mut self, reason: &str, now: u64) {
        self.strikes += 1;
        self.last_strike_at = Some(now);
        
        // Progressive punishment policy
        let quarantine_secs = match self.strikes {
            0 => 60,       // Should not happen, but 1 minute just in case
            1 => 120,      // 2 minutes
            2 => 1800,     // 30 minutes
            _ => 86400,    // 24 hours (3+ strikes)
        };
        
        self.quarantined_until = Some(now + quarantine_secs);
        
        tracing::warn!(
            "[STRIKES] ⚠️  {}:{} received strike #{} for {} | quarantined for {}s",
            self.ip, self.port, self.strikes, reason, quarantine_secs
        );
    }
    
    /// Fix B: Check if this peer is currently quarantined
    ...
    /// Fix C: Update validated height (only increases, never decreases)
    pub fn update_validated_height(&mut self, height: u64) {
        if height > self.last_validated_height {
            self.last_validated_height = height;
            tracing::debug!(
                "[VALIDATED-HEIGHT] {}:{} validated up to height={}",
                self.ip, self.port, height
            );
        }
    }
    
    /// Update sync state based on peer's height relative to local height
    /// Called periodically and when heights change
    pub fn update_sync_state(&mut self, local_height: u64, now: u64) {
        const CONSENSUS_RANGE: u64 = 10; // Peers within ±10 blocks are consensus peers
        const SYNC_TIMEOUT_SECS: u64 = 300; // 5 minutes without block request = stale
        const GRACE_PERIOD_SECS: u64 = 300; // 5 minute grace period for new connections
        
        let peer_height = self.last_reported_height.unwrap_or(0);
        let previous_state = self.sync_state;
        
        // Classify based on height relationship
        if peer_height > local_height {
            // Peer is ahead - we sync from them
            self.sync_state = PeerSyncState::Ahead;
        } else if local_height.abs_diff(peer_height) <= CONSENSUS_RANGE {
            // Peer is within consensus range
            self.sync_state = PeerSyncState::Consensus;
        } else {
            // Peer is behind us - check if syncing or stale
            
            // Grace period: new connections start as SYNCING for first 5 minutes
            if let Some(conn_start) = self.connection_started_at {
                if now < conn_start + GRACE_PERIOD_SECS {
                    self.sync_state = PeerSyncState::Syncing;
                    return;
                }
            }
            
            // Check if we've received a block request recently
            if let Some(last_req) = self.last_block_request_at {
                if now < last_req + SYNC_TIMEOUT_SECS {
                    // Active syncing - received request within last 5 minutes
                    self.sync_state = PeerSyncState::Syncing;
                } else {
                    // No recent requests - stale
                    self.sync_state = PeerSyncState::Stale;
                }
            } else {
                // Never requested blocks - stale
                self.sync_state = PeerSyncState::Stale;
            }
        }
        
        // Log state transitions
        if previous_state != self.sync_state {
            tracing::info!(
                "[SYNC-STATE] {}:{} {:?} → {:?} | peer_h={} local_h={} lag={}",
                self.ip, self.port, previous_state, self.sync_state,
                peer_height, local_height, local_height.saturating_sub(peer_height)
            );
        }
    }
    
    /// Record that this peer requested blocks from us
    pub fn record_block_request(&mut self, now: u64) {
        self.last_block_request_at = Some(now);
        tracing::debug!(
            "[BLOCK-REQ] {}:{} requested blocks (sync_state={:?})",
            self.ip, self.port, self.sync_state
        );
    }
}
...
impl Peer {
    ...
    pub fn new(ip: String, port: u16, ebid: String) -> Self {
        Self {
            ...
            // Consensus validation defaults
            pow_params_hash: None,
            pow_msg_version: None,
            ...
            // Validated height tracking
            last_validated_height: 0,
            fork_consistent: false,
            
            // Directional sync state tracking (new peers start in Syncing grace period)
            sync_state: PeerSyncState::Syncing,
            last_block_request_at: None,
            connection_started_at: Some(std::time::SystemTime::now()
    ...
    pub fn is_chain_compatible(
        ...
        pow_params_hash: Option<&str>,
        pow_msg_version: Option<u32>,
    ) -> bool {
        // Fix A: Consensus fingerprint hard gate - check PoW params first
        if let Some(expected_pow_hash) = pow_params_hash {
            match &self.pow_params_hash {
                Some(peer_pow_hash) if peer_pow_hash != expected_pow_hash => {
                    tracing::warn!(
                        "[COMPAT] ❌ {}:{} INCOMPATIBLE: pow_params_hash mismatch | expected={} | got={}",
                        self.ip, self.port, expected_pow_hash, peer_pow_hash
                    );
                    return false;
                }
                None => {
                    tracing::warn!(
                        "[COMPAT] ❌ {}:{} INCOMPATIBLE: missing pow_params_hash | expected={}",
                        self.ip, self.port, expected_pow_hash
                    );
                    return false;
                }
                _ => {} // matches, continue
        ...
        // Fix A: Check POW message version compatibility
        if let Some(expected_pow_msg) = pow_msg_version {
            match self.pow_msg_version {
                Some(peer_pow_msg) if peer_pow_msg != expected_pow_msg => {
                    tracing::warn!(
                        "[COMPAT] ❌ {}:{} INCOMPATIBLE: pow_msg_version mismatch | expected={} | got={}",
                        self.ip, self.port, expected_pow_msg, peer_pow_msg
                    );
                    return false;
                }
                None => {
                    tracing::warn!(
                        "[COMPAT] ❌ {}:{} INCOMPATIBLE: missing pow_msg_version | expected={}",
                        self.ip, self.port, expected_pow_msg
                    );
                    return false;
                }
                _ => {} // matches, continue
...
impl PeerManager {
    ...
    /// Add a strike to a peer by address (ip:port)
    pub async fn add_peer_strike(&self, peer_addr: &str, reason: &str) {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs();
        ...
        let mut peers = self.peers.write().await;
        // Find peer by ip:port address
        if let Some(peer) = peers.values_mut().find(|p| format!("{}:{}", p.ip, p.port) == peer_addr) {
            peer.add_strike(reason, now);

            // Persist updated peer to database
            let ebid = format!("{}:{}", peer.ip, peer.port);
            if let Some(db) = &self.db {
                if let Ok(encoded) = bincode::serialize(&*peer) {
                    let _ = db.insert(ebid.as_bytes(), encoded);
                }
    ...
    /// Update peer's validated height after successful block acceptance (Fix C)
    ...
    pub async fn update_peer_chain_identity(
        ...
        if let Some(peer) = peers.get_mut(ebid) {
            ...
            peer.pow_params_hash = if pow_params_hash.is_empty() { None } else { Some(pow_params_hash) };
            peer.pow_msg_version = if pow_msg_version == 0 { None } else { Some(pow_msg_version) };
            peer.height = Some(chain_height);
            peer.last_reported_height = Some(chain_height);
            peer.last_height_updated_at = Some(std::time::Instant::now());
    ...
    /// ONLY counts AHEAD and CONSENSUS peers for mining gate (SYNCING/STALE peers excluded)
    pub async fn consensus_quorum(&self) -> ConsensusQuorum {
        ...
        // Fix A: Compute POW params for validation
        let pow_hash = crate::p2p::connection::compute_pow_params_hash();
        let pow_msg_ver = crate::p2p::connection::POW_MSG_VERSION;
        ...
        for p in peers.values() {
            ...
            if p.is_chain_compatible(
                ...
                // CRITICAL: Only count AHEAD and CONSENSUS peers toward mining gate
                // SYNCING and STALE peers are excluded to prevent mining blockage
                match p.sync_state {
                    PeerSyncState::Ahead | PeerSyncState::Consensus => {
                        compatible_peers += 1;
                    ...
                    PeerSyncState::Syncing | PeerSyncState::Stale => {
                        // Peers behind us don't count toward mining consensus
                        // but they're not incompatible either
                    }
                }
            } else {
    ...
    /// Blocking version of consensus_quorum for non-async contexts (e.g., mining eligibility)
    /// CRITICAL: Use this in sync code paths where async is not available
    pub fn consensus_quorum_blocking(&self) -> ConsensusQuorum {
        ...
        let peers = match self.peers.try_read() {
            Ok(p) => p,
            Err(_) => {
                // Lock contention - return empty quorum rather than deadlock
                tracing::warn!("[QUORUM] Lock contention in consensus_quorum_blocking, returning empty quorum");
                return ConsensusQuorum {
                    compatible_peers: 0,
                    incompatible_peers: 0,
                    min_compatible_height: None,
                    max_compatible_height: None,
                };
            }
        };
    ...
    /// Update all connected peers' sync states based on current heights
    /// Called periodically to classify peers as AHEAD/CONSENSUS/SYNCING/STALE
    pub async fn update_all_sync_states(&self) {
        let local_height = {
        ...
        tracing::info!(
            "[SYNC-STATE-UPDATE] Updating sync states: local_h={} total_peers={} connected={}",
            local_height, total_peers, connected_peers
        );
        
        for peer in peers.values_mut() {
            if peer.state == PeerState::Connected {
                peer.update_sync_state(local_height, now);
            }
        }
    }
    
    /// Enforce max 3 SYNCING peers policy
    /// When more than 3 peers are in SYNCING state, demote the ones with longest idle time to STALE
    pub async fn enforce_syncing_peer_limit(&self) {
        const MAX_SYNCING_PEERS: usize = 3;
        
        let mut peers = self.peers.write().await;
        
        // Collect all SYNCING peers with their last request time
        let mut syncing_peers: Vec<(String, u64)> = peers
            .iter()
            .filter(|(_, p)| p.state == PeerState::Connected && p.sync_state == PeerSyncState::Syncing)
            .map(|(ebid, p)| {
                let last_req = p.last_block_request_at.unwrap_or(0);
                (ebid.clone(), last_req)
            })
            .collect();
        
        if syncing_peers.len() <= MAX_SYNCING_PEERS {
            return; // Under limit, nothing to do
        }
        
        // Sort by last request time (oldest first)
        syncing_peers.sort_by_key(|(_, last_req)| *last_req);
        
        // Demote the oldest ones to STALE (keep only the 3 most recent)
        let to_demote = syncing_peers.len() - MAX_SYNCING_PEERS;
        
        for (ebid, _) in syncing_peers.iter().take(to_demote) {
            if let Some(peer) = peers.get_mut(ebid) {
                let peer_height = peer.last_reported_height.unwrap_or(0);
                let local_height = {
                    let g = crate::CHAIN.lock();
                    g.blocks.len() as u64
                };
                ...
                tracing::info!(
                    "[SYNC-LIMIT] Demoting peer {}:{} from SYNCING → STALE (limit={}, idle)",
                    peer.ip, peer.port, MAX_SYNCING_PEERS
                );
                
                peer.sync_state = PeerSyncState::Stale;
            }
        }
    }
    
    /// Record block request from a peer by address
    /// Public method for tracking sync activity
    pub async fn record_peer_block_request(&self, peer_addr: &str) {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        ...
        let mut peers = self.peers.write().await;
        if let Some(peer) = peers.values_mut().find(|p| p.socket_addr() == peer_addr) {
            peer.record_block_request(now);
        }
    }
    
    /// Get peer sync state by address
    /// Public method for connection pruning
    pub async fn get_peer_sync_state(&self, peer_addr: &str) -> Option<PeerSyncState> {
        let peers = self.peers.read().await;
        peers.values()
            .find(|p| p.socket_addr() == peer_addr)
            .map(|p| p.sync_state)
    }

    /// Find the highest known remote height among compatible peers.
    /// Used by auto-sync to determine the best chain to follow.
    /// Fix C: Now uses VALIDATED heights only, not advertised heights
    pub async fn best_remote_height(&self) -> Option<u64> {
        ...
        // Fix B: Get current timestamp for quarantine check
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        ...
        peers
            .values()
            .filter(|p| {
                // Only use connected, compatible, non-quarantined peers
                p.state == PeerState::Connected
                    && !p.is_quarantined(now) // Fix B: exclude quarantined
            ...
            .filter_map(|p| {
                // Use advertised height for sync decisions
                // The peer's advertised height tells us where the network is,
                // even if we haven't validated blocks from them yet
                p.height
            })
            .max()
    }
    ...
    pub fn try_best_remote_height(&self) -> Option<u64> {
        ...
        peers
            ...
            .filter_map(|p| {
                // Fix C: Use validated height, not advertised height
                if p.last_validated_height > 0 {
                    Some(p.last_validated_height)
                } else {
                    p.height // Fallback for initial sync
                }
            })
            .max()
    }
