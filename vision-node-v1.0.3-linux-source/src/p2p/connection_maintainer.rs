...
pub async fn run_connection_maintainer(
    ...
    loop {
        ...
        // === HEIGHT-BASED CONNECTION PRUNING ===
        // Disconnect peers that are too far behind to be useful
        prune_lagging_peers().await;
...
/// Prune peers that are too far behind to be useful
/// 
/// This implements height-based connection management with sync state awareness:
/// - AHEAD peers: Never pruned (we sync from them)
/// - CONSENSUS peers: Never pruned (within ±10 blocks)
/// - SYNCING peers: Only pruned if >750 blocks behind OR no progress in 10 minutes
/// - STALE peers: Pruned if >100 blocks behind
/// - Tracks stuck peers (same height for >10 minutes)
/// - Adds to cooldown via dial_tracker for automatic backoff
async fn prune_lagging_peers() {
    // Configurable thresholds
    let max_height_lag_stale: u64 = std::env::var("VISION_MAX_HEIGHT_LAG")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(100); // STALE peers: prune if >100 blocks behind
    const MAX_HEIGHT_LAG_SYNCING: u64 = 750; // SYNCING peers: prune if >750 blocks behind
    const STUCK_DURATION_SECS: u64 = 600; // 10 minutes
    ...
    info!(
        "[SYNC-PRUNE] Starting pruning cycle: local_h={} thresholds=(STALE>{}, SYNCING>{})",
        local_height, max_height_lag_stale, MAX_HEIGHT_LAG_SYNCING
    );
    
    // Update all peer sync states first
    crate::PEER_MANAGER.update_all_sync_states().await;
    
    // Enforce max 3 SYNCING peers limit
    crate::PEER_MANAGER.enforce_syncing_peer_limit().await;
    
    // Get all connected peers with heights
    let p2p_mgr = crate::P2P_MANAGER.clone_inner();
    let peer_infos = p2p_mgr.get_peer_info().await;
    
    let mut pruned = 0;
    let mut stuck_detected = 0;
    
    for peer in peer_infos {
        let peer_height = peer.height;
        let height_lag = local_height.saturating_sub(peer_height);
        let addr_str = peer.address.clone();
        
        // Get peer's sync state
        let sync_state = crate::PEER_MANAGER
            .get_peer_sync_state(&addr_str)
            .await
            .unwrap_or(crate::p2p::peer_manager::PeerSyncState::Stale);
        
        // Pruning logic based on sync state
        let should_prune = match sync_state {
            crate::p2p::peer_manager::PeerSyncState::Ahead |
            crate::p2p::peer_manager::PeerSyncState::Consensus => {
                // NEVER prune AHEAD or CONSENSUS peers
                false
            }
            crate::p2p::peer_manager::PeerSyncState::Syncing => {
                // SYNCING peers: NEVER prune (let them sync indefinitely)
                // Mining gate already excludes them from compatible peer count
                false
            }
            crate::p2p::peer_manager::PeerSyncState::Stale => {
                // STALE peers: prune if >100 blocks behind
                height_lag > max_height_lag_stale
            }
        };
        
        if should_prune {
            info!(
                "[CONN_PRUNING] 🔻 Disconnecting peer: {} @ h{} (lag: {} blocks, state: {:?})",
                addr_str, peer_height, height_lag, sync_state
            );
            
            // Disconnect gracefully (removes from connections map)
            p2p_mgr.remove_peer(&addr_str).await;
            
            // Add to cooldown via dial tracker
            crate::p2p::dial_tracker::record_dial_failure(
                addr_str.clone(),
                "height_lagging".to_string(),
                "conn_pruning".to_string(),
            );
        ...
        // Check for stuck peers (same height for >10 minutes)
        if is_peer_stuck(&addr_str, peer_height, STUCK_DURATION_SECS).await {
            info!(
                "[CONN_PRUNING] ⏸️ Disconnecting stuck peer: {} @ h{} (no progress for {}m)",
                addr_str, peer_height, STUCK_DURATION_SECS / 60
            );
    ...
    if pruned > 0 || stuck_detected > 0 {
        info!(
            "[CONN_PRUNING] 📊 Pruned {} lagging, {} stuck peers (local_height: {})",
            pruned, stuck_detected, local_height
        );
    }
}

/// Track peer heights over time to detect stuck peers
static PEER_HEIGHT_TRACKER: once_cell::sync::Lazy<
    tokio::sync::RwLock<HashMap<String, PeerHeightState>>
> = once_cell::sync::Lazy::new(|| {
    tokio::sync::RwLock::new(HashMap::new())
});

#[derive(Clone)]
struct PeerHeightState {
    last_height: u64,
    last_update: u64, // Unix timestamp
    stuck_count: u8,
}

/// Check if a peer is stuck at the same height
async fn is_peer_stuck(addr: &str, current_height: u64, threshold_secs: u64) -> bool {
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
        .as_secs();
    
    let mut tracker = PEER_HEIGHT_TRACKER.write().await;
    
    match tracker.get_mut(addr) {
        Some(state) => {
            if state.last_height == current_height {
                // Same height - check duration
                let duration = now - state.last_update;
                if duration >= threshold_secs {
                    state.stuck_count += 1;
                    return true;
                }
            } else {
                // Height changed - reset
        ...
        None => {
            ...
            tracker.insert(addr.to_string(), PeerHeightState {
