...
pub fn is_mining_eligible() -> bool {
    ...
    // Gate 2: Peer count enforcement (mainnet floor: 3 peers minimum for MINING)
    ...
    if now > last_log + 15 && connected_peers < effective_min_peers {
        ...
        tracing::info!(
            "[MINING-GATE] Mining peer requirements: env_min={} mainnet_floor={} effective_floor={} connected={} (sync needs only {})",
            min_peers, MAINNET_MIN_PEERS_FOR_MINING, effective_min_peers, connected_peers, MIN_PEERS_FOR_SYNC
        );
    }
    ...
    // If we're more than 2 blocks behind, we're in active sync mode - pause mining
    // Tight threshold prevents mining wrong fork while sync catches up
