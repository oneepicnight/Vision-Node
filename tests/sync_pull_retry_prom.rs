...
async fn sync_pull_retry_prom() {
    ...
    let r = client.post(format!("{}/sync/pull", base)).json(&body).send().await.unwrap();
    ...
    assert!(body.contains("vision_sync_pull_retries_total"), "missing retries metric");
    if let Some(line) = body.lines().find(|l| l.starts_with("vision_sync_pull_retries_total ")) {
    ...
    assert!(body.contains("vision_sync_pull_failures_total"), "missing failures metric");
