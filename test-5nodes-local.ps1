...
"@ | Out-File -FilePath $launchScript -Encoding UTF8
    ...
    foreach ($node in $nodes) {
        try {
            $debug = Invoke-RestMethod "http://localhost:$($node.HttpPort)/api/p2p/debug" -TimeoutSec 3
    ...
    Write-Host "[HINT] Check dial failures with: curl http://localhost:7070/api/p2p/debug | jq .dial_failures" -ForegroundColor Cyan
